...
def install_hadoop(nodesDF,masternode,nodemanagers,osMemory):
    ...
    Remote("cd {0} && mv hadoop-* hadoop".format(wget_destination),hosts=all_hosts,connection_params={'user': 'root'}).run() ## move hadoop to a directory without the version
    ## CREATE THE MASTER FILE
    with open("hadoop-resources/tmp/master",'w') as f:
        f.write(list(masternode)[0] + "\n")
    Put(hosts=masternode,local_files=["hadoop-resources/tmp/master"],remote_location=hadoop_conf + "/master").run()
    ## CREATE THE SLAVES FILE
    with open("hadoop-resources/tmp/slaves","w") as f:
        for node in nodemanagers:
            f.write(node + "\n")
    Put(hosts=masternode,local_files=["hadoop-resources/tmp/slaves"],remote_location=hadoop_conf + "/slaves").run()
    ## CORE_SITE.XML
    ...
    ## MAP_RED.XML
    Put(hosts=all_hosts,local_files=[path_mapred_site_template],remote_location=hadoop_conf + "/mapred-site.xml",connection_params={'user': 'root'}).run()
    ### CHANGE THE JAVA_HOME ENVIRONMENTAL VARIABLE FOR HADOOP
    Remote("perl -pi -e 's,.*JAVA_HOME.*,export JAVA_HOME=/usr/lib/jvm/java-7-openjdk-amd64/jre,g' {0}/hadoop-env.sh".format(hadoop_conf),hosts=all_hosts,connection_params={'user': 'root'}).run()
    ### WE GIVE PERMISSIONS TO THE G5K USER
    Remote("chown -R {0}:users /opt/hadoop*".format(g5k_configuration.get("g5k_user")),hosts=all_hosts,connection_params={'user': 'root'}).run()
