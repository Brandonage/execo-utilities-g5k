...
def install_gmone(master,slaves):
    ...
    Put(hosts=master.union(slaves),local_files=["gmone-resources/GMonE-MC"],remote_location="/opt").run()
    ...
    Put(hosts=master.union(slaves),local_files=["gmone-resources/tmp/manager.conf"],remote_location="/opt/GMonE-MC/manager.conf").run()
    ...
    Put(hosts=master.union(slaves),local_files=["gmone-resources/templates/monitor.conf.template"],remote_location="/opt/GMonE-MC/monitor.conf").run()
    #   change permissions so g5k_user can operate with GMONE
    Remote("chown -R {0}:users /opt/GMonE-MC".format(g5k_configuration.get("g5k_user")),hosts=master.union(slaves),connection_params={'user': 'root'}).run()

def start_gmonedb(master):
    Remote("cd /opt/GMonE-MC/ && nohup /opt/GMonE-MC/gmonedb.sh {{{host}}} /opt/GMonE-MC/manager.conf > /dev/null 2>&1",hosts=master,connection_params={'user':g5k_configuration.get('g5k_user')},process_args={"shell":True}).start()

def start_gmonemon(slaves):
    start_dstat(slaves)
    Remote("cd /opt/GMonE-MC/ && nohup /opt/GMonE-MC/gmonemon.sh {{{host}}} /opt/GMonE-MC/monitor.conf > /dev/null 2>&1",hosts=slaves,connection_params={'user':g5k_configuration.get('g5k_user')},process_args={"shell":True}).start()
